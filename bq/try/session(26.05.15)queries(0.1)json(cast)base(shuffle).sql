-- Contains all required logic for signature generation/level parsing
-- note: it is actually more shuffle efficient to read blob directly than the pass the blob as an array of parts
  ...
  from (
    select len,a||b str from (
      select len,
        ...
        if(scan=0, if(len > 1,safe.format('%t',array_last(blob)),''),
...
create or replace table function tmp.mapJsonObjects1(input table< schema string, parts array<json>,sig int,rel int,type string>, scan bool) as (
  ...
  select array(
    select as struct idx,tmp.getJsonObjects1((jsn).to_json_string()) levels 
    from unnest(parts) as jsn with offset as idx
  ) as parts from shuf

  -- select tmp.getJsonObjects1((jsn).to_json_string()) levels  from shuf
...
sigs as (
  ...
  select tmp.getJsonSigFromArray(blob,typeof(blob),tail=>null,scan=>0).*
  from real 

)

select (parts).array_last().levels.array_last().children.array_last().data from tmp.mapJsonObjects1(table sigs,scan=>true)
