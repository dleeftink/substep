...
select str,length(str) len,array(
  ...
  |> aggregate 
    ...
    group by cat,if(entry,depth,greatest(deep,nest)) a ,--if(entry,eth,least(dth - if(sym = '}',1,0),nth - if(sym = ']',1,0))) b  --, 
      slot - if(closer,1,0) as slot
  |> select as struct * except(cat,a)
