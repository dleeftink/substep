...
CREATE OR REPLACE FUNCTION tmp.mapStringSafeGuards5(str STRING, chars string, codes array<int>) AS ((
  ...
  WITH chunks AS (
    ...
    FROM UNNEST(
      --SPLIT((str).regexp_replace(r'(\')([^\']*)(\')|(")([^"]*)(")',concat('\\1\\4\x0E\x0F',r'\2\5','\x0E\\3\\6')), '\x0E')
      (str).regexp_replace(r'(\'[^\']*\'|"[^"]*")',concat('\x0E\x0F',r'\1','\x0E')).SPLIT('\x0E')
  ...
  select if(chars is null,str,array_to_string(
    array(SELECT 
      IF(is_content, 
        left(part,1)||tmp.fixJsonSafeGuards5((part).trim('"\''),chars,codes)||right(part,1),
      part),  
    FROM chunks)  
  ,''))
...
CREATE OR REPLACE FUNCTION tmp.mapStringSafeGuards6(str STRING, chars string, codes array<int>) AS ((
  ...
  WITH chunks AS (
    SELECT 
      part part, off,
      open in ('\'','"') /*and open = close*/ AS is_content,
      open,close
    FROM (select part,off,left(part,1) open, right(part,1) close from UNNEST(
      (str).regexp_extract_all(r'\'[^\']*\'|"[^"]*"|[^\'"]+')
      /*(str).TEXT_ANALYZE(
        analyzer=>'PATTERN_ANALYZER',
        analyzer_options=>'{"patterns": ['||('[\'][^\']*[\']|["][^"]*["]|[^\'"]+').to_json_string()||']}'
      )*/
    ) AS part WITH OFFSET off)
  )
  ...
  select if(chars is null,str,array_to_string(
    array(SELECT 
      IF(is_content, 
        open||tmp.fixJsonSafeGuards5((part).trim('"\''),chars,codes)||close,
      part),  
    FROM chunks)  
  ,''))
...
CREATE temp FUNCTION getStructuralChars(str string) as (
...
prep as (
  ...
  select sel, strs[0].str as key, strs[1].str as val from (
    select sel, array(select as struct STR SRC,(str).(tmp.mapStringSafeGuards5)(chars,codes) str,chars,codes
      from unnest([
        ...
        struct(safe.format('%T',blob) as str,'()[],<>#\'"' as chars,[0x11, 0x13, 0x02, 0x03, 0x1F, 0x01, 0x04, 0x1B, 0x0E, 0x0F] as codes)]) -- can be quite sparse with encoding here. as we extract double quotes anyways
    ) strs from real
  )
