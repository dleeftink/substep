-- this version includes strategies for extracting the array contexts
-- some slight upfront compute costs
...
create or replace table function tmp.mapJsonFragmentTypes(input table <part string, ctx string, idx int>) as ( 
  from input
  ...
  |> extend 
      ...
      --(longtail) in ('[{','[[').if(longtail,null) ctx,
  ...
  |> set key = (key).rtrim(': '),dat = (run).if(clip,dat)
  |> select idx,cat,sym,ctx,key,sep,dat,cap

);

create or replace table function tmp.useJsonPartialFinaliser(input table <part string, off int>) as (

  from input
  |> extend case 
     when (part).regexp_contains(r'[\[\{]\s*[\[\{]$') then struct('['||right(part,1) as ctx, [(part).left(length(part) - 1),right(part,1)] as ent)
     else null end as dup

  |> LEFT JOIN UNNEST (dup.ent) as ent with offset seq
  |> set part = coalesce(ent,part)
  --|> cross join unnest(dup.ent) ent with offset seq
  --|> set part = ent
  |> extend (SUM(LENGTH(part)) OVER (ORDER BY off,seq ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING) + 1 ).ifnull(0) idx,
  |> select part,dup.ctx,idx
...
create or replace function tmp.getJsonObjects3(str string) as (
  ...
  array(
    ...
    |> call tmp.useJsonPartialFinaliser()
...
line as (
  ...
  select '{"objarry":[  {"id":1},{"id":2},[["nested"]]],"":[ 1, 2,3,{"nested": true  } ,{"null":true},, ,,, ,, ,4,5,  ", ", " "  ], "okay":false}' as str,true as wide
),
