...
create or replace function tmp.fixJsonEntity(ent string) as (
  coalesce(
    safe.parse_json('{'||ent||'}'),
    safe.parse_json('['||ent||']'),
    safe.parse_json('['||(ent)
      .REGEXP_REPLACE(r'("(?:[^"\\]|\\.)*")|(^|,)\s*(,|$)', r'\1\2null\3')
      .REGEXP_REPLACE(r'("(?:[^"\\]|\\.)*")|(^|,)\s*(,|$)', r'\1\2null\3')
      .replace('nullnull','')
    ||']')
  )
...
create or replace table function tmp.mapJsonFragmentTypes(input table <part string, off int>) as ( 
  ...
  |> extend (SUM(len) OVER (ORDER BY off ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING) + 1 ).ifnull(0) idx,
  |> extend (part).trim('\t\n\r ').nullif('') bare 
  |> where bare is not null

  |> extend (bare).rtrim('\t\n\r ,') as clip
  |> extend (clip).right(1) as tail --, (clip).right(size - (clip).rtrim('\t\n\r[ {').length()).translate('\t\n\r ','') longtail
  |> extend 
      (tail) in ('{','}') as obj,
      (tail) in ('[',']') as arr,
  ...
  |> extend sym is null as rec
  |> extend (clip != sym or rec).if(case when obj then clip||'}' when arr then clip||']' else clip end,null) as ent
  |> set ent = tmp.fixJsonEntity(ent)
  |> extend json_keys(ent,1)[safe_offset(0)] as key
  |> extend (rec).if(coalesce(ent[key],ent),null) as dat
  |> extend (rec and key is null).if(json_value_array(ent).array_length(),null) as cap
  |> set key = (key is not null and (key).length() = 0).if('undefined',key)
  |> select idx,cat,sym,key,sep,dat,len, cap

);

create or replace aggregate function tmp.getJsonObjectNodes2(
  pick BOOL, obj STRUCT<slot INT64, open INT64, close INT64, head STRING, type STRING, key STRING, data JSON, tail STRING, entry BOOL>
) as (
  struct(
...
create or replace function tmp.getJsonObjects3(str string, pick int,rgx string) as (
